 ...
 */
export const PromptInput = ({ onSubmit, onChange, isDisabled = false, placeholder = '', slashMenuActive = false, }) => {
    ...
    useInput((input, key) => {
        ...
        if (key.return && !key.shift) {
            if (slashMenuActive)
                return; // Let SlashMenu handle it
            if (value.trim()) {
                onSubmit(value);
    ...
    const renderInput = () => {
        if (isDisabled) {
            return (_jsx(Text, { color: theme.fg.muted, children: placeholder || '(waiting...)' }));
        }
        if (!value && !isDisabled) {
            return (_jsx(Text, { color: theme.fg.dim, children: placeholder || '' }));
        }
        // Show text with cursor indicator
        const beforeCursor = value.slice(0, cursorPos);
        const atCursor = value[cursorPos] || ' ';
        const afterCursor = value.slice(cursorPos + 1);
        return (_jsxs(Text, { children: [_jsx(Text, { color: theme.fg.primary, children: beforeCursor }), _jsx(Text, { color: theme.bg.default, backgroundColor: theme.fg.primary, children: atCursor }), _jsx(Text, { color: theme.fg.primary, children: afterCursor })] }));
    };
    return (_jsxs(Box, { flexDirection: "row", borderStyle: "single", borderColor: theme.border.default, paddingTop: 0, paddingBottom: 0, paddingLeft: 1, paddingRight: 1, marginTop: 1, flexShrink: 0, children: [_jsx(Text, { color: theme.accent.brand, bold: true, children: '> ' }), _jsx(Box, { flexDirection: "column", flexGrow: 1, children: renderInput() })] }));
};

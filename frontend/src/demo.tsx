...
const DEMO_PERMISSION: PermissionRequest = {
  id: 'perm-1',
...
function DemoApp() {
  ...
  return (
    <Box flexDirection="column" width="100%" height="100%">
      <StatusBar agentState={agentState} columns={dims.columns} />

      <Box flexDirection="column" flexGrow={1} overflow="hidden">
        {state === 0 ? (
          <WelcomeScreen columns={dims.columns} />
        ) : (
          <MessageList
            messages={messages}
            terminalHeight={dims.rows}
            isThinking={state === 2}
            thinkingContent=""
            waitingForResponse={state === 3}
          />
        )}
      </Box>
      ...
      {showPermission && (
        <PermissionDialog
          ...
          onAlwaysAllow={() => setShowPermission(false)}
        />
      )}
      ...
      <PromptInput
        ...
        placeholder={state === 0 ? 'Type a message...' : ''}
      />

      <HelpBar columns={dims.columns} />

      <Box paddingLeft={1}>
        <Text color={theme.fg.dim}>
          Tab: next state ({stateLabels[(state + 1) % 5]}) | Current: {stateLabels[state]}
        </Text>
      </Box>
    </Box>
  );

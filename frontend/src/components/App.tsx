...
import { Message, AgentState, PermissionRequest } from '../lib/types.js';
...
export const App: React.FC = () => {
  ...
  const [availableSessions, setAvailableSessions] = useState<Array<{id: string, title: string, timestamp: string}>>([]);
  ...
  useEffect(() => {
    ...
    const onThinkingStream = (params: any) => {
    ...
    const onAssistantStream = (params: any) => {
    ...
    const onAssistantMessage = (params: any) => {
      const content = params.content || '';
    ...
    const onToolResult = (params: any) => {
      ...
      setMessages(prev => [...prev, {
        ...
        toolName: params.name,
        toolState: (params.state || 'success') as 'success' | 'error',
        timestamp: Date.now(),
      }]);
    };

    // Agent state changes
    const onAgentStateChanged = (params: any) => {
    ...
    // Permission requests
    const onPermissionRequested = (params: any) => {
  ...
  const handleSlashCommand = useCallback((command: string) => {
    ...
    switch (cmd) {
      ...
      case '/model':
        ...
        ipc.send('get_models', {}).then((result: any) => {
      ...
      case '/sessions':
        ...
        ipc.send('list_sessions', {}).then((result: any) => {
      ...
      case '/compact':
        if (messages.length === 0) {
          ...
          ipc.send('compact_conversation', {}).then((result: any) => {
            if (result.error) {
              setMessages(prev => [...prev, {
                role: 'system' as const,
  ...
  return (
    <Box flexDirection="column" width="100%" height="100%">
      {/* Status Bar - Top */}
      <StatusBar agentState={agentState} columns={dimensions.columns} />

      {/* Persistent Header - ALWAYS visible, NEVER conditional */}
      <PersistentHeader />

      {/* Main Content Area */}
      <Box flexDirection="column" flexGrow={1}>
        {/* WelcomeScreen: OUTSIDE overflow, flexShrink={0} so Ink can never compress it */}
        <Box flexShrink={0} width="100%" alignItems="center">
          <WelcomeScreen columns={dimensions.columns} />
        </Box>
        {/* Messages: only this area has overflow hidden */}
        <Box flexDirection="column" flexGrow={1} overflow="hidden">
          <MessageList
            messages={messages}
            terminalHeight={dimensions.rows}
            isThinking={agentState.status === AGENT_THINKING}
      ...
      {showSessionModal && (
        <SessionSelectModal
          ...
          currentSessionId={agentState.sessionId}
          onSelect={(sessionId) => {
            const ipc = getIPC();
            ipc.send('load_session', { session_id: sessionId }).then((result: any) => {
              if (result.error) {
                setMessages(prev => [...prev, {
                  role: 'system' as const,
      ...
      <PromptInput
        ...
        isDisabled={inputDisabled || showModelModal || showSessionModal}
        placeholder=""
        slashMenuActive={showSlashMenu}
      />

      {/* Help Bar */}
      <HelpBar columns={dimensions.columns} />
    </Box>
  );
};
